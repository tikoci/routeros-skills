...
description: "Talk to a RouterOS device from a shell or from TypeScript with centrs (@tikoci/centrs) — a CLI + Bun/TS hub that resolves a router's address, credentials, and protocol for you, validates RouterOS-shaped commands before running them, and returns one structured envelope over REST, native API, SSH, MAC-Telnet, SFTP, and MNDP. Use when: reading or changing config on a real router or CHR; running a RouterOS CLI command non-interactively; checking whether a RouterOS command is well-formed before sending it; moving files to or from a device; the same operation has to run across several routers. For booting the CHR itself use routeros-quickchr; for RouterOS documentation use the rosetta MCP or routeros-fundamentals."
---
